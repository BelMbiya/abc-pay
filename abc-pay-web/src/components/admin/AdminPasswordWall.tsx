...
import { changeAdminPassword, clearAdminToken } from "@/lib/admin-auth";

/** Mur de 1re connexion admin : changement du mot de passe provisoire avant tout accès. */
export function AdminPasswordWall({ onDone }: { onDone: () => void }) {
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-gray-100 px-5 py-10">
      <div className="w-full max-w-[420px] rounded-2xl bg-white p-6 shadow-hero">
        ...
        <p className="mt-1.5 text-[13px] leading-relaxed text-gray-500">Pour ta sécurité, remplace le mot de passe provisoire avant d&apos;accéder à l&apos;espace d&apos;administration.</p>
        ...
        <button type="button" onClick={() => { clearAdminToken(); window.location.href = "/admin-connexion"; }} className="mt-4 flex w-full items-center justify-center gap-1.5 text-[12.5px] font-bold text-gray-500 hover:text-ink">

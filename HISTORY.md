...
## 2026-08-05 — #7 suivi comptes + #11 audit exceptions/notifs admin + gestion FAQ (Lot D bouclé)
- **#7 Suivi user admin** : `GET /admin/users/{user}/transactions` (`AdminUserController@transactions` → `TransactionHistoryService::forUser`, 500 max). Front `AdminUserSheet` : bouton « Voir toute la trace (N) » + liste scrollable (date · type · canal · établissement · contrepartie · statut). Note sécurité : compte payeur = **OTP, pas de mot de passe** → leviers = Déconnecter / Bloquer.
- **#11 Audit exceptions** : filet `render(Throwable)` dans `bootstrap/app.php` — toute exception non gérée sur `api/*` → enveloppe uniforme `error.code=server_error` (message user-friendly, **jamais** le message brut ni la stack ; détail `debug` seulement si `APP_DEBUG`). Les erreurs HTTP normales (404/403/401, validation) gardent leur rendu.
- **#11 Notifications admin** : table `admin_notifications` (fil PARTAGÉ par l'équipe admin — les admins vivent dans `admins`, id disjoint de `users`), `AdminNotification`, `AdminNotificationService` (`push/latest/unreadCount/markAllRead`), `AdminNotificationController` + `GET/POST /admin/notifications[/read]`. Émission best-effort (n'interrompt jamais le flux) : `FraudService::flag()` (gravité→niveau high/critical…), `SupportService::open()/panic()` (urgent→critical), filet exception (system/critical). **Angle mort UX comblé** : `SupportService::respond()` notifie le titulaire du ticket. Front : `AdminNotificationBell` (réutilise `NotificationCenter`, token admin, `basePath=/api/v1/admin`) dans `AdminShell` (header mobile + flottant desktop) ; niveaux `warning`/`critical` ajoutés au composant partagé (`bg-fee-bg`/`text-gold-600`, `ShieldAlert`). Tests `AdminNotificationTest` (5). Suite **127/127**.
- **Gestion FAQ (façon avis)** : table `faqs` (category/question/answer/position/is_published), `Faq`, `FaqController@publicIndex` (publiées, ordonnées) + `Admin\FaqAdminController` (index/store/update/destroy) + `FaqSaveRequest`. Routes `GET /faqs/public` + admin `GET/POST/PATCH/DELETE /admin/faqs`. Front : `lib/faq-api.ts`, page **`/admin/faq`** (CRUD : ajouter/éditer en feuille, publier/masquer, supprimer, ordre) + nav admin « FAQ ». **Injection landing** : `landing-logic` remplace la FAQ statique de `#grid-faq` par les entrées publiées (repli sur la FAQ bilingue du design si aucune ; rendu FR seul comme les avis) + rebranchement du marqueur +/–. Page `/faq` payeur : fetch des FAQ publiées (repli statique). Tests `FaqTest` (4). Suite backend **131/131, 441 assertions**. Vérifié E2E : entrée publiée → `/faqs/public` → `#grid-faq` la montre, accordéon +/– OK.

## 2026-08-05 — Auth payeur corrigée : inscription délibérée, plus de compte fantôme
- **Problème** : recevoir un code créait un compte VIDE (`User::create(['phone'])`) et connectait, même pour un numéro inconnu — l'inscription n'était pas un acte volontaire et la connexion « fabriquait » le compte. Modèle retenu (choix user) : **OTP + inscription réelle**.
- **Backend** : `AuthService::loginWithPhoneToken($idToken, $intent, $profile)` — le compte n'est créé (signup) qu'**AVEC une identité** ; `FirebaseAuthRequest` exige `profile.name` (min:2) en signup (garde-fou anti-compte-fantôme, `validation_failed` sinon) + champs KYC optionnels validés. **Numéro déjà inscrit → simple connexion** (l'OTP prouve la possession) sans jamais écraser l'identité existante (anti-usurpation). `login` sur numéro inconnu → `account_not_found` (404). `AuthController` passe `profile`. `AuthTest` réécrit + 2 nouveaux (signup-sans-nom refusé ; signup-sur-existant n'écrase pas) → **18 tests auth**, suite **133/133 (450 assertions)**.
- **Front** : `verifyOtp(phone, code, intent, profile?)`. **`/connexion` = connexion seule** (plus de `?nouveau=1`) : numéro inconnu → panneau « Aucun compte pour ce numéro » + CTA vers `/inscription?tel=…` (pré-remplit). **`/inscription` = seul point de création** (sans session préalable) : nom + numéro requis, KYC facultatif → conditions → **OTP → le compte est créé À CE MOMENT** avec l'identité → session ouverte → `/`. Landing « Créer un compte » → `/inscription` (nav-map + href markup). Après connexion : toujours `/` (KYC complétable depuis le profil, plus de boucle vers inscription).
- **Vérifié** : live API (numéro inconnu→404, signup-sans-nom→422) ; `AuthTest` (signup-avec-nom crée + login réussit) ; DOM `/connexion` (titre « Connexion » + « Crée ton compte ») & `/inscription` (numéro+nom requis, KYC « facultatif »). Build + eslint OK. ⚠️ Rate-limiter `throttle:auth` renvoie encore le format brut Laravel sur 429 (HttpException non ré-enveloppée) — cosmétique, à uniformiser un jour.

## 2026-08-05 — Limites d'affichage configurables + suppression d'établissement (admin)
- **Bug UI** : le bouton « Ajouter » de `/admin/faq` prenait toute la largeur (`Button` est `fullWidth` par défaut) → `fullWidth={false}` (compact, vérifié 110px).
- **Limites d'affichage public configurables** : `SettingsService` — `landing_faq_limit` (défaut 8) & `landing_reviews_limit` (défaut 6), bornés 1–50, cache + `all()`. `UpdateSettingsRequest` + `SettingsController` acceptent les deux. Appliqués À LA SOURCE : `FaqController@publicIndex` (`->limit(faqLimit())`) et `ReviewController@publicIndex` (`reviewsLimit()`). Front : `settings-api` (`updateLandingLimits`), section « Affichage public (landing) » dans `/admin/parametres` (2 champs), et **retrait du `slice(0,6)` en dur** de la landing (le réglage fait foi). Test `FaqTest` +1 (5 publiées + limite 2 → 2 renvoyées).
- **Suppression d'établissement (super-admin)** : `EstablishmentProvisioningService::delete()` avec **garde-fou intégrité financière** — refus (`EstablishmentActionException`) si l'établissement a des transactions (→ suspendre plutôt). Sinon cascade en transaction (fee_items, reminders, fee_schedules, learners, fee_types, establishment_staff, comptes de connexion dédiés `whereDoesntHave('transactions')`, puis l'établissement — les FK sont en `restrict`, cascade manuelle obligatoire). `EstablishmentAdminController@destroy` → `cannot_delete` (422) sur refus. Route `DELETE /admin/establishments/{establishment}`. Front : `deleteEstablishment()` + **« Zone de danger »** dans `ConfigureEstablishmentSheet` (bouton + confirmation, message serveur affiché si refus). Tests `EstablishmentProvisioningTest` +3 (suppr OK sans tx ; refus 422 avec tx ; 401 sans auth). Suite **137/137 (466 assertions)**.
- **Vérifié E2E (live)** : bouton FAQ 110px ; section limites (8/6) présente ; suppression — happy path (create→delete 200, disparu de la liste) ET garde-fou (établissement avec tx → 422 « …historique de transactions… Suspends-le »). Build + eslint OK.

## 2026-08-05 — Espace établissement : nom affiché, avis, retrait des membres + confirmations globales
- **Nom de l'établissement dans l'accueil back-office** : `StaffAuthService` ajoute `establishment_name` au payload de login ; `StaffUser` (front) le porte ; `/etablissement` affiche le nom en titre (repli « Tableau de bord »). Vérifié : « Institut Supérieur de Commerce (ISC) ».
- **Avis des établissements** : `ReviewSheet` rendu réutilisable (props `base`/`token`/`rolePlaceholder`) ; carte « Votre avis sur abc pay » + bouton « Donner mon avis » dans `/etablissement/parametres` → `POST /api/v1/staff/reviews` (author_type=staff, statut pending, modéré avant publication landing). Vérifié live (201 pending).
- **Invitation de membres RETIRÉE** (décision produit) : route `POST /staff/members` supprimée, `StaffMemberController@store` + `StoreStaffMemberRequest` + `StaffDirectoryService::addMember` supprimés (liste `GET` conservée, consultation seule). Front : `InviteStaffSheet` supprimé, `inviteStaffMember` retiré, page params sans bouton « Inviter ». Test : `StaffMemberTest` → POST renvoie **405** (fonctionnalité retirée).
- **Fenêtres de confirmation généralisées** (demande user) : nouveau `ConfirmProvider`/`useConfirm` (`components/ui/ConfirmDialog.tsx`, modale portal, Échap/voile = annuler, bouton rouge si `danger`), monté dans les 3 shells (payeur/back-office/admin). Appliqué à TOUTES les actions irréversibles/dangereuses : suspendre/réactiver un établissement, bloquer/débloquer/déconnecter/supprimer un compte, supprimer un établissement, supprimer une FAQ, geler son compte (support payeur). Les anciens confirms « inline » (deux boutons) migrés vers la modale pour l'uniformité.
- **Vérifié E2E (live)** : dialogue de confirmation à la suspension (titre + Annuler/Réactiver, annulation non destructive) ; nom établissement en accueil ; params sans « Inviter » + avec « Donner mon avis » ; avis staff 201 pending. Backend **135/135 (462 assertions)** · ESLint (`src` complet) + build OK.
